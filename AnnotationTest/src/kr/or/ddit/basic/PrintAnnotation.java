...
/**
	...
	프로그램 소스코드 안에서 다른 프로그램 또는 컴파일러 등을 위한 정보를 
	  미리 약속된 형식으로 포함시킨 것 (JDK 1.5부터 지원함.)
	...
	애너테이션 타입요소의 규칙
	 1.요소의 타입은 기본형, String, enum, anootation, class만 허용된다.
	 2.()안에 매개변수를 선언할 수 없다.
	 3.예외를 선언할 수 없다.
	 4.요소의 타입에 매개변수(제너릭타입문자)를 사용할 수 없다.
 */

@Target({ElementType.METHOD, ElementType.TYPE}) // 적용가능한 대상 지정함.
@Retention(RetentionPolicy.RUNTIME) // 유지되는 기간을 지정함.(기본은 CLASS 임.)
